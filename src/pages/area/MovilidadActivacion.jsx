...
function MovilidadActivacion() {
  const [rutinas, setRutinas] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");

  const cargarRutinas = async () => {
    const rutinasMovilidadActivacion = await getRutinasPorArea(
      "Movilidad/Activacion"
    );
    setRutinas(rutinasMovilidadActivacion);
  };
  ...
  return (
    <div className="p-6 bg-neutral-800">
      ...
      <p className="text-lg mb-8 text-white">
        Explora las rutinas de Movilidad y Activación subidas por la comunidad.
      </p>
      ...
      <div className="space-y-6 mt-5">
        {rutinasFiltradas.length > 0 ? (
          ...
          <p className="text-white">
            No hay rutinas disponibles para Movilidad y Activación en este
            momento.
          </p>
        )}
      </div>
...
export default MovilidadActivacion;
